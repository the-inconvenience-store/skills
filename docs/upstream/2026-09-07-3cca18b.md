...
| Local | `ec25238f32125603c550e4f97393a9fac9f5d3f2` | `/Users/samstevens/skills-dev/skills` |
...
Reconciliation is complete: all 37 upstream skills are either mapped (27 changed, 3 unchanged) or covered by 7 explicit policy ignores; all 33 local skills are mapped or one of 3 intentional fork-only skills. All 25 upstream docs pages are mapped and changed; 3 additional local docs pages are fork-only. There are no duplicate mapping targets, unclassified skills or docs, or stale ignore rules.

### Adopt candidates

- No upstream skill implementation is a direct adoption candidate. The meaningful deltas run in the other direction: local policy and workflow additions absent upstream.
- Selectively curate verified operational guidance from the 25 rewritten upstream docs pages. Useful additions include sharper trigger boundaries, prerequisites, observable success signals, and known failure modes. Do not copy the pages wholesale.
- Three docs-derived behavior gaps merit separate implementation review: ensure `/implement` reviews committed/current work rather than an invisible working tree, prohibit recursive skill delegation in `code-review` and `research`, and validate tracker label/dependency commands in `setup-inconvenient-skills`, `triage`, and `to-tickets`. These are upstream-documented rough edges, not fixes shipped in the upstream skill files, so reproduce each locally before changing behavior.

### Defer

- Defer the upstream docs rewrite as a batch. It removes the locally required Quickstart and Source frame, expands most pages substantially, uses upstream branding and links, and sometimes describes upstream defects already fixed locally (notably diagnostic redaction and final verification).
- Defer the docs-derived behavior gaps until separately scoped and verified. If adopted, update the matching local docs; update `ask-inconvenient` too when routing or flow relationships change.

### Ignore

- Ignore punctuation-only churn: em-dash substitutions, table alignment, YAML quoting, blank lines, and comment rewording account for the upstream-side changes in the 27 mapped skill diffs. Also ignore `ask-matt`, `setup-matt-pocock-skills`, Matt Pocock display names, and upstream repository branding in favor of the configured fork mappings.
- Preserve the local `engineering-principles`, `guardrails`, and `verification` skills and docs, plus their integrations in the router, setup, design, implementation, debugging, review, specification, and ticket workflows. These are intentional fork capabilities, not deletion candidates.
- Preserve local Beads support in `setup-inconvenient-skills`; its absent upstream template is the only file-level mapped-skill difference that may represent an upstream deletion, and the fork intentionally still supports that tracker.
