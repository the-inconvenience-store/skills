...
For another ecosystem, use its current official documentation and say that this skill has no bundled stack reference. Do not invent a rule set from memory.
...
1. Run every new or changed lint, format-check, and aggregate command.
...
- existing violations or unsupported ecosystems left over;
