...
- **`/setup-project`** — run when starting a repo or overhauling its **guardrails**. It inspects the project, uses curated stack-specific rule baselines to propose one lint, format, CI, and hook setup for your approval, then installs and verifies only what you accept. For a new React application it also asks whether to adopt Bulletproof React-inspired application standards. Re-run it when the stack or quality strategy changes; smaller adjustments belong in the existing config.
