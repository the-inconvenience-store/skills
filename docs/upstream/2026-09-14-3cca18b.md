...
| --- | --- | --- |
| Upstream | `3cca18b368ae95cdbdebbff572ccafa662551015` | https://github.com/mattpocock/skills.git |
| Local | `b296e647b770e0134e05073657d1b01a8a377dd4` (dirty working tree) | `/Users/samstevens/skills-dev/skills` |
...
Implementation run. Upstream has not moved since the 2026-09-07 audit — both reconcile against `3cca18b` — so the adoption question this run was not "what is new upstream" but "which upstream-documented defects still bite here". Three were closed; see _Implemented this run_. Reconciliation is complete and unchanged by that work: all 37 upstream skills are either mapped (27 changed, 3 unchanged) or covered by 7 explicit policy ignores; all 33 local skills are mapped or one of 3 intentional fork-only skills. All 25 upstream docs pages are mapped and changed; 3 further local docs pages are fork-only. No new upstream skills or docs, no duplicate mapping targets, no unclassified entries, no stale ignore rules. No `config.json` change was needed: no skill was added, renamed, or newly classified.

### Implemented this run

Each fix came from a defect the upstream docs describe but the upstream skill files do not fix, so each was reproduced locally before changing behavior. All three widen the fork's divergence deliberately.

- **Unbounded review fan-out.** Both `code-review` sub-agent briefs now forbid re-entering the skill or spawning further agents; `research` delegates exactly once and does the reading in-thread when it cannot delegate, which also closes the opposite failure where a no-re-delegation policy left the run silently doing nothing.
- **Labels that were never created.** `docs/agents/triage-labels.md` is a mapping, and writing it creates nothing; GitHub then rejects `gh issue create --label <missing>` outright and loses the whole issue rather than degrading to an unlabelled one. `setup-inconvenient-skills` now checks the tracker for the agreed strings and creates the missing ones (including `wayfinder:*`) after asking, since that writes to a shared tracker; `triage` and `to-tickets` verify before their first label write.
- **Blocking edges written as prose.** The GitHub tracker template and `to-tickets` now prefer the native relationships — `gh issue create --blocked-by` / `--parent`, `gh issue edit --add-blocked-by` / `--add-sub-issue`, all verified against `gh` 2.98 — with the `gh api` dependency call kept as the pre-2.94 path and the body "Blocked by" line demoted to an explicit fallback. A body-text edge is invisible to the frontier query on a tracker that supports the real thing.
- **Ordering fix in `implement`.** It now commits before calling `code-review` and names the fixed point, because `git diff <fixed-point>...HEAD` cannot see staged or working-tree changes and would review an empty diff as clean.

Docs pages for all six changed skills were re-synced. One dead link was fixed in passing: `docs/engineering/research.md` pointed at a non-existent `to-prd`. `ask-inconvenient` needed no change — no skill was added or removed and no routing relationship moved.

### Remaining differences, and why each is intentional

- **The 27 changed mapped skills.** Re-verified this run against every diff: each upstream-side change is line rewrapping, em-dash-to-colon substitution, table realignment, or upstream branding. Where content genuinely differs the fork is ahead, and this run widened that lead in six of them.
- **The 25 changed docs pages.** Still a batch defer. The upstream rewrite is genuinely richer in structure, but it drops the locally required Quickstart and Source frame, carries upstream branding and links, and documents defects the fork has now fixed. Mine it selectively; do not adopt wholesale.
- **3 fork-only skills** (`engineering-principles`, `guardrails`, `verification`) and their docs, plus their integrations across the router, setup, design, implementation, debugging, review, specification, and ticket workflows. Intentional fork capabilities, not deletion candidates.
- **Beads support in `setup-inconvenient-skills`.** Its absent-upstream template remains the only file-level mapped-skill difference that may represent an upstream deletion; the fork still supports that tracker.
- **The `ask-matt` / `setup-matt-pocock-skills` renames and Matt Pocock branding**, handled by the configured mappings.

No meaningful upstream change remains unadopted.
...
| `skills/in-progress/retro` | `in-progress/retro` | 1 modified |
| `skills/in-progress/setup-ts-deep-modules` | `in-progress/setup-ts-deep-modules` | 2 modified |
| `skills/productivity/grilling` | `skills/grilling` | 1 modified |
...
| Upstream | Local | File delta |
| --- | --- | --- |
| `—` | `skills/engineering-principles` | 23 absent upstream |
| `—` | `skills/guardrails` | 14 absent upstream |
| `—` | `skills/verification` | 6 absent upstream |
...
| Upstream | Local | File delta |
| --- | --- | --- |
| `—` | `docs/engineering/engineering-principles.md` | 1 absent upstream |
| `—` | `docs/engineering/guardrails.md` | 1 absent upstream |
| `—` | `docs/engineering/verification.md` | 1 absent upstream |
...
-  short_description: "Configure repo workflows, guardrails, and verification"
...
### `skills/in-progress/retro` → `in-progress/retro`
...
~~~~diff
--- local/in-progress/retro/SKILL.md
+++ upstream/skills/in-progress/retro/SKILL.md
@@ -1,6 +1,6 @@
 ---
 name: retro
...
~~~~diff
--- local/skills/grilling/SKILL.md
+++ upstream/skills/productivity/grilling/SKILL.md
@@ -4,8 +4,6 @@
 ---
...
<details>
<summary><code>SKILL.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/SKILL.md
+++ upstream/(absent)/SKILL.md
@@ -1,65 +0,0 @@
----
...
<details>
<summary><code>agents/openai.yaml</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/agents/openai.yaml
+++ upstream/(absent)/agents/openai.yaml
@@ -1,3 +0,0 @@
-interface:
-  display_name: "Engineering Principles"
-  short_description: "Apply shared principles to engineering decisions"
~~~~

</details>

<details>
<summary><code>references/boundary-discipline.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/boundary-discipline.md
+++ upstream/(absent)/references/boundary-discipline.md
@@ -1,9 +0,0 @@
-# Boundary Discipline
-
...
<details>
<summary><code>references/build-the-lever.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/build-the-lever.md
+++ upstream/(absent)/references/build-the-lever.md
@@ -1,9 +0,0 @@
-# Build the Lever
-
-**Apply when:** work is repeated, broad, error-prone, or uncertain enough that a reusable tool can perform or prove it.
-
-Build the smallest script, codemod, generator, fixture, checker, or driving route that turns the work into a deterministic operation. The lever should be the artifact a reviewer can run, not a private convenience whose output must be trusted.
-
-Build it before the bulk work when every later unit benefits. Keep it only when repetition, risk, or future maintenance repays its cost; otherwise use a throwaway script and remove it during cleanup.
-
-A lever does not justify automation around a one-line edit. Its value is repeatability, coverage, or stronger evidence.
~~~~

</details>

<details>
<summary><code>references/encode-lessons-in-structure.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/encode-lessons-in-structure.md
+++ upstream/(absent)/references/encode-lessons-in-structure.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/exhaust-the-design-space.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/exhaust-the-design-space.md
+++ upstream/(absent)/references/exhaust-the-design-space.md
@@ -1,9 +0,0 @@
-# Exhaust the Design Space
-
-**Apply when:** a design is expensive to reverse, lacks a strong repository precedent, and admits several structurally different shapes.
-
-Produce at least two genuinely different designs or prototypes before selecting one. Whole-shape alternatives vary ownership, interface, data model, or interaction; changing a method name inside one architecture is not an alternative.
-
-For module interfaces, call the Skill tool with `codebase-design` and use its Design It Twice process. For observable state or UI questions, call the Skill tool with `prototype`. Compare alternatives by the decisions they expose, not by presentation polish.
-
-Skip this for mechanical work and decisions already settled by a strong local precedent. Exploration earns its cost only before a consequential choice closes options.
~~~~

</details>

<details>
<summary><code>references/experience-first.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/experience-first.md
+++ upstream/(absent)/references/experience-first.md
@@ -1,9 +0,0 @@
-# Experience First
-
-**Apply when:** product scope or user experience is being traded against implementation convenience.
-
-Choose the smallest coherent experience that solves the user's problem well. Do not expose internal constraints as extra user steps, confusing modes, leaky terminology, or rough behavior merely because they make the implementation easier.
-
-When several experiences are genuinely plausible, call the Skill tool with `grilling` to resolve the product decision. When seeing and using alternatives would produce better evidence than discussion, call the Skill tool with `prototype`.
-
-Fewer polished capabilities beat a wider set of incomplete ones. This principle cannot invent scope that the user did not ask for.
~~~~

</details>

<details>
<summary><code>references/fix-root-causes.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/fix-root-causes.md
+++ upstream/(absent)/references/fix-root-causes.md
@@ -1,9 +0,0 @@
-# Fix Root Causes
-
...
<details>
<summary><code>references/foundational-thinking.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/foundational-thinking.md
+++ upstream/(absent)/references/foundational-thinking.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/guard-the-context-window.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/guard-the-context-window.md
+++ upstream/(absent)/references/guard-the-context-window.md
@@ -1,9 +0,0 @@
-# Guard the Context Window
-
-**Apply when:** a phase boundary, long investigation, large output, or parallel fan-out threatens the context needed for sound judgment.
-
-Keep primary-source reasoning in the current session while the next phase still depends on it and the work fits. Delegate independent bulk reading or checks, then retain compact findings and artifact pointers rather than raw output.
-
-Move context only at a phase boundary. Use a handoff when the work must travel to another harness, directory, colleague, or forked side task; use a fresh ticket context when the ticket is self-contained.
-
-Summaries are lossy. Do not compact or hand off merely to make the window look tidy, and do not keep bulk payloads in the lead session when only their conclusions are needed.
~~~~

</details>

<details>
<summary><code>references/keep-execution-unblocked.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/keep-execution-unblocked.md
+++ upstream/(absent)/references/keep-execution-unblocked.md
@@ -1,9 +0,0 @@
-# Keep Execution Unblocked
-
-**Apply when:** a reversible execution choice is about to become a human checkpoint.
-
-Facts are the agent's job. Inspect the repository, run the experiment, or use the available tool instead of asking the user to supply observable information. Proceed on reversible implementation details when the accepted behavior and repository precedent make the choice clear, then present the result for asynchronous correction.
-
-Decisions are the user's job when they change product direction, scope, experience, risk acceptance, or another genuine preference. Irreversible external actions also require confirmation.
-
-A running investigation blocks only questions that depend on its result. Continue every independent branch of the frontier rather than stopping the whole task.
~~~~

</details>

<details>
<summary><code>references/laziness-protocol.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/laziness-protocol.md
+++ upstream/(absent)/references/laziness-protocol.md
@@ -1,9 +0,0 @@
-# Laziness Protocol
-
...
<details>
<summary><code>references/make-operations-idempotent.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/make-operations-idempotent.md
+++ upstream/(absent)/references/make-operations-idempotent.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/migrate-callers-then-delete-legacy-apis.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/migrate-callers-then-delete-legacy-apis.md
+++ upstream/(absent)/references/migrate-callers-then-delete-legacy-apis.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/minimize-reader-load.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/minimize-reader-load.md
+++ upstream/(absent)/references/minimize-reader-load.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/model-behavior-structurally.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/model-behavior-structurally.md
+++ upstream/(absent)/references/model-behavior-structurally.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/outcome-oriented-execution.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/outcome-oriented-execution.md
+++ upstream/(absent)/references/outcome-oriented-execution.md
@@ -1,13 +0,0 @@
...
<details>
<summary><code>references/prove-it-works.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/prove-it-works.md
+++ upstream/(absent)/references/prove-it-works.md
@@ -1,9 +0,0 @@
-# Prove It Works
-
-**Apply when:** work is about to be called complete.
-
-Identify the observable claim, then check the real artifact directly. Run the public path, read the actual value, inspect the produced file or diff, and verify the full input-to-output chain. Compilation, tests, cached representations, and agent reports are supporting signals rather than substitutes for the claimed result.
-
-For user-facing application behavior, call the Skill tool with `verification`. Its `VERIFIED`, `NOT VERIFIED`, and `INCONCLUSIVE` verdicts preserve the evidence boundary.
-
-Trust delegated artifacts only after inspecting or exercising them yourself. When a check passes too easily, test the observation method before concluding the system is correct.
~~~~

</details>

<details>
<summary><code>references/redesign-from-first-principles.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/redesign-from-first-principles.md
+++ upstream/(absent)/references/redesign-from-first-principles.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/separate-before-serializing-shared-state.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/separate-before-serializing-shared-state.md
+++ upstream/(absent)/references/separate-before-serializing-shared-state.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/sequence-verifiable-units.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/sequence-verifiable-units.md
+++ upstream/(absent)/references/sequence-verifiable-units.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/subtract-before-you-add.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/subtract-before-you-add.md
+++ upstream/(absent)/references/subtract-before-you-add.md
@@ -1,9 +0,0 @@
...
<details>
<summary><code>references/type-system-discipline.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/engineering-principles/references/type-system-discipline.md
+++ upstream/(absent)/references/type-system-discipline.md
@@ -1,9 +0,0 @@
...
~~~~diff
--- local/skills/guardrails/SKILL.md
+++ upstream/(absent)/SKILL.md
@@ -1,126 +0,0 @@
----
-name: guardrails
...
~~~~diff
--- local/skills/guardrails/agents/openai.yaml
+++ upstream/(absent)/agents/openai.yaml
@@ -1,3 +0,0 @@
-interface:
-  display_name: "Guardrails"
-  short_description: "Guide project guardrails and reproducible checks"
~~~~

</details>

<details>
<summary><code>references/agent-hooks.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/agent-hooks.md
+++ upstream/(absent)/references/agent-hooks.md
@@ -1,32 +0,0 @@
-# Agent hooks
-
...
<details>
<summary><code>references/bulletproof-react.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/bulletproof-react.md
+++ upstream/(absent)/references/bulletproof-react.md
@@ -1,81 +0,0 @@
...
<details>
<summary><code>references/git-hooks.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/git-hooks.md
+++ upstream/(absent)/references/git-hooks.md
@@ -1,44 +0,0 @@
-# Git hooks
-
...
<details>
<summary><code>references/go.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/go.md
+++ upstream/(absent)/references/go.md
@@ -1,45 +0,0 @@
-# Go
-
...
<details>
<summary><code>references/project-lifecycle.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/project-lifecycle.md
+++ upstream/(absent)/references/project-lifecycle.md
@@ -1,51 +0,0 @@
-# Project lifecycle
-
...
<details>
<summary><code>references/react.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/react.md
+++ upstream/(absent)/references/react.md
@@ -1,39 +0,0 @@
-# React
-
...
<details>
<summary><code>references/research-ecosystem.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/research-ecosystem.md
+++ upstream/(absent)/references/research-ecosystem.md
@@ -1,80 +0,0 @@
...
<details>
<summary><code>references/rules/dependencies.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/rules/dependencies.md
+++ upstream/(absent)/references/rules/dependencies.md
@@ -1,56 +0,0 @@
-# Dependency rules
-
...
<details>
<summary><code>references/rules/go.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/rules/go.md
+++ upstream/(absent)/references/rules/go.md
@@ -1,98 +0,0 @@
-# Go rules
-
...
<details>
<summary><code>references/rules/react.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/rules/react.md
+++ upstream/(absent)/references/rules/react.md
@@ -1,106 +0,0 @@
-# React rules
-
...
<details>
<summary><code>references/rules/typescript.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/rules/typescript.md
+++ upstream/(absent)/references/rules/typescript.md
@@ -1,93 +0,0 @@
...
<details>
<summary><code>references/typescript.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/guardrails/references/typescript.md
+++ upstream/(absent)/references/typescript.md
@@ -1,47 +0,0 @@
...
~~~~diff
--- local/skills/verification/SKILL.md
+++ upstream/(absent)/SKILL.md
@@ -1,48 +0,0 @@
----
-name: verification
...
~~~~diff
--- local/skills/verification/agents/openai.yaml
+++ upstream/(absent)/agents/openai.yaml
@@ -1,3 +0,0 @@
-interface:
-  display_name: "Verification"
-  short_description: "Prove behavior on the real user-facing surface"
~~~~

</details>

<details>
<summary><code>references/create.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/verification/references/create.md
+++ upstream/(absent)/references/create.md
@@ -1,52 +0,0 @@
...
<details>
<summary><code>references/maintain.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/verification/references/maintain.md
+++ upstream/(absent)/references/maintain.md
@@ -1,69 +0,0 @@
...
<details>
<summary><code>references/project-format.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/verification/references/project-format.md
+++ upstream/(absent)/references/project-format.md
@@ -1,58 +0,0 @@
...
<details>
<summary><code>references/run.md</code> — absent upstream</summary>


~~~~diff
--- local/skills/verification/references/run.md
+++ upstream/(absent)/references/run.md
@@ -1,59 +0,0 @@
...
<details>
<summary><code>engineering-principles.md</code> — absent upstream</summary>


~~~~diff
--- local/docs/engineering/engineering-principles.md
+++ upstream/(absent)
@@ -1,53 +0,0 @@
-Quickstart:
-
...
<details>
<summary><code>guardrails.md</code> — absent upstream</summary>


~~~~diff
--- local/docs/engineering/guardrails.md
+++ upstream/(absent)
@@ -1,52 +0,0 @@
-Quickstart:
-
...
<details>
<summary><code>verification.md</code> — absent upstream</summary>


~~~~diff
--- local/docs/engineering/verification.md
+++ upstream/(absent)
@@ -1,55 +0,0 @@
-Quickstart:
-
...
| Skill | `skills/in-progress/retro` | `in-progress/retro` | Adopted upstream draft; fork keeps drafts under in-progress/ |
