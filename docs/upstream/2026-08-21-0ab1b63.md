# Upstream audit — 2026-08-21

## Provenance

| Side | Revision | Source |
| --- | --- | --- |
| Upstream | `0ab1b63a410a03d3627979a109c8695de27af954` | https://github.com/mattpocock/skills.git |
| Local | `00c1c2eb3ccba92ae19d2f08feb3c0424c6d39e9` (dirty working tree) | `/Users/samstevens/skills-dev/skills` |
...
Refreshed after implementation. Every upstream skill is mapped or explicitly ignored, every local skill is mapped, and there are no duplicate targets, unclassified paths, stale ignore rules, new upstream skills, or local skills absent upstream. The reconciliation now has 24 changed mapped skills and 4 unchanged; all 25 docs pages remain changed under the fork's documentation policy.

### Implemented (adopt)

- **`wait-what` now handles multi-context repositories.** It follows `CONTEXT-MAP.md` to the relevant `CONTEXT.md` instead of assuming the root glossary is the only one. `docs/productivity/wait-what.md` now explains that branch.
- **`grilling` now separates questions inside a round.** Its template demonstrates two questions with a horizontal rule between them, without changing the whole-frontier interview behavior. `docs/productivity/grilling.md` now calls out the visual separation. The skill itself now reconciles as unchanged.
- **`wayfinder` now uses explicit cross-skill calls throughout.** The three stale Ticket Types descriptions now tell the agent to call the Skill tool for `research`, `prototype`, and the separate `grilling` / `domain-modeling` calls, matching `.agents/invocation.md` and the operative steps later in the same skill. `docs/engineering/wayfinder.md` now describes the research subagent accurately.

No manifest, README index, router, invocation metadata, or audit configuration change was needed. No skill was added, removed, renamed, promoted, rerouted, or reclassified, and none of the adopted paths contains upstream branding.
...
- **Upstream's repository-wide em-dash removal is style churn.** It accounts for nearly every remaining skill-file diff, including the three mapped drafts, plus script comments, Markdown tables, and metadata. The fork keeps its existing prose and formatting. Upstream's related YAML quoting fix is unnecessary while the local descriptions retain valid em-dash wording rather than colon-space scalars.
- **Branding and invocation policy stay localized.** Keep `ask-inconvenient`, `setup-inconvenient-skills`, `the-inconvenience-store/skills`, matching `openai.yaml` display names, and user-invocation settings. Keep the local router's accurate statement that `diagnosing-bugs` ends at cleanup; upstream still advertises a post-mortem handoff that its debugging skill no longer performs.
- **Beads support remains fork-only.** `setup-inconvenient-skills/issue-tracker-beads.md` and the connected setup branches have no upstream counterpart by design. This is maintained local capability, not stale content to delete.
- **The local `implement` policy remains more specific.** It commits as work proceeds using Conventional Commit-style messages; upstream only says to commit.
- **The local `wizard` template keeps its dead-color cleanup.** Upstream still defines an unused `RED` value in both color branches.
- **All 25 docs pages keep the local publication format.** They retain the Quickstart, update, localized source link, concise orientation, and `ask-inconvenient` routing required by `.agents/writing-docs.md`. The three adopted pages were re-synced locally rather than replaced with upstream's longer AI Coding Dictionary-oriented versions.
- **Seven ignored upstream skills remain intentionally excluded.** The four `skills/misc` utilities and three `in-progress` writing drafts still exist upstream, so none of their ignore rules is stale. The mapped local drafts ship no docs pages and differ only in punctuation or formatting.

No upstream deletion strands local content. Every remaining changed mapping is covered by the fork policies above.

### Verification

The refreshed audit completed successfully. `claude plugin validate . --strict`, YAML frontmatter checks, docs frame and absolute-link checks, branding scans, and `git diff --check` pass. Claude, Codex, and marketplace manifest versions remain synchronized at `1.3.2`.

The prescribed Codex validator reports only its existing policy conflict on 14 user-invoked skills: it requires Claude's `disable-model-invocation` field to be absent or false, while this repository deliberately sets it true and pairs it with Codex's `policy.allow_implicit_invocation: false` per `.agents/invocation.md`. It reports no other error, and this implementation does not weaken user-only invocation to satisfy that conflicting constraint.
