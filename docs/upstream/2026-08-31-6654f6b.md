# Upstream audit — 2026-08-31

## Provenance

| Side | Revision | Source |
| --- | --- | --- |
| Upstream | `6654f6b60cd9d5be8b54c6fafe44346dabeb3b76` | https://github.com/mattpocock/skills.git |
| Local | `dafd8339b669476c001a2195c2d150ec77583582` (dirty working tree) | `/Users/samstevens/skills-dev/skills` |
...
Refreshed after implementation. Every upstream skill is mapped or explicitly ignored, every local skill is mapped, and there are no duplicate targets, unclassified paths, stale ignore rules, new upstream skills, or local skills absent upstream. The reconciliation now has 26 changed mapped skills and 4 unchanged; all 25 docs pages remain changed under the fork's documentation policy.

Upstream moved three commits since the [2026-08-24 audit](./2026-08-24-5b15a47.md) (`5b15a47..6654f6b`), all of them the same new draft: `8fa1886` adds the `retro` skill and its OpenAI config, `3ec8e23` tweaks its description and lists it in the upstream README, `6654f6b` adds an "Information access" category to it. Nothing else. Before this run, the detailed-diff sections of this report were byte-identical to the previous report apart from the 75 lines describing that skill, which confirms mechanically that no other upstream behavior landed.

### Implemented (adopt)

- **`retro` adopted as an in-progress draft.** It runs a retrospective on a coding session aimed at the agent's **environment** rather than the code: read the session's primary sources, then rank improvement candidates across seven named categories — navigation, automated checks, coding standards, global `AGENTS.md` bloat, tool economy, no-op instructions, and information access. Its load-bearing idea is the implementation-vs-review split: the review agent has no context pressure because it only receives a diff, so coding standards belong in review rather than in every implementer's context window.

  Nothing in the fork covered this. `wait-what` and `handoff` operate inside a live session; nothing looked back at a finished one to change the harness. It stays a draft because the "search through session logs on this machine" step is unspecified and it assumes `CODING_STANDARDS.md` and `AGENTS.md` conventions this fork does not itself impose — which is where upstream keeps it too.

  Added `in-progress/retro/SKILL.md` and `agents/openai.yaml`, listed it in `in-progress/README.md`, and recorded an explicit `skill_mappings` entry in the audit configuration. It ships no docs page, no top-level README entry, no plugin manifest entry, and no router entry, as the `in-progress/` bucket requires. Its invocation metadata already matched fork policy (`disable-model-invocation: true` with `allow_implicit_invocation: false`), and a case-insensitive branding scan of the adopted paths found no upstream identifiers, so no vocabulary substitution was needed.
...
No top-level README, manifest, router, or invocation-metadata change was needed. No promoted skill was added, removed, renamed, promoted, rerouted, or reclassified.
