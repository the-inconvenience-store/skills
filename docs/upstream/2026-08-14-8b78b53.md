# Upstream audit — 2026-08-14

## Provenance

| Side | Revision | Source |
| --- | --- | --- |
| Upstream | `8b78b531ab965735c5dc74f6f7a219e1e37326df` | https://github.com/mattpocock/skills.git |
| Local | `9e32cfa9a0f71ae4d9c571065502ab6105b90f1a` (dirty working tree) | `/Users/samstevens/skills-dev/skills` |
...
Refreshed after implementation. Every upstream skill is mapped or ignored, no local skill is absent upstream, and no ignore rule has gone stale. Upstream landed exactly one commit series since this fork's last sync at `84fdeff` — it has been adopted. The 8 remaining changed skills and 25 changed docs pages are all deliberate fork divergence.

### Implemented this run

- **`domain-modeling` description** (upstream `8b78b53`) — the trigger moved from intent (*"when the user wants to pin down domain terminology … or when another skill needs to maintain the domain model"*) to observable actions: *"Use when discussing codebase terminology, writing or editing a CONTEXT.md, or recording or editing an ADR."* This widens model invocation to fire on `CONTEXT.md` and ADR edits. `docs/engineering/domain-modeling.md`'s "When to reach for it" beat was re-synced to match. The skill now reconciles as unchanged.
- No config change was needed: `domain-modeling` was already an explicit same-name mapping, not a newly classified skill. No upstream branding appeared in the adopted text.
- `ask-inconvenient`'s router line and the `README.md` entry already describe terminology and ADR work, so both stayed accurate.

### Intentional fork divergence — no action

- **Branding renames** — `ask-matt` → `ask-inconvenient`, `setup-matt-pocock-skills` → `setup-inconvenient-skills`, plus every `/setup-matt-pocock-skills` call-out inside `code-review`, `to-spec`, `to-tickets`, `triage`, and `wayfinder`, the two `openai.yaml` display names, and the `mattpocock/skills` → `the-inconvenience-store/skills` column header in `triage-labels.md`. Covered by the configured mappings.
- **Beads issue tracker** (`skills/setup-inconvenient-skills/issue-tracker-beads.md`, added in `49a3e5a`) — a fork-only tracker option, with its matching branches in `SKILL.md`'s detection, Section A, and file list. Reads as "absent upstream" because upstream never had it, not because upstream deleted it.
- **Docs pages (all 25)** — the fork keeps the Quickstart / `npx skills add` / Source frame that upstream dropped in `3314257`, and writes shorter pages against this repo's [`.agents/writing-docs.md`](../../.agents/writing-docs.md) instead of upstream's longer prose with AI Coding Dictionary links. House style, not a missed sync; no upstream behaviour rides in these diffs.
- **`wizard/template.sh`** — upstream defines `RED` in both colour branches and never uses it; the fork dropped the dead pair. Nothing to adopt unless a stage template starts needing it.
...
No new upstream skills, no upstream deletions leaving local content stranded, no unclassified paths, and no stale ignore rules — `skills/misc/*` and the three `in-progress` writing drafts all still exist upstream. Upstream's `skills/deprecated/` is now empty apart from its README, so it contributes nothing to classify.
