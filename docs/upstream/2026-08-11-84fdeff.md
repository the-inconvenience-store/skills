...
| Local | `a0fad8a4046abfe0c3e30da7ff16d84975b47f9f` (dirty working tree) | `/Users/samstevens/skills-dev/skills` |
...
All meaningful upstream behavior is now represented locally. The automated reconciliation found no new or absent skills; 20 mapped skills are byte-for-byte unchanged and 8 retain deliberate fork differences. Those differences are the `ask-inconvenient` and `setup-inconvenient-skills` renames and their references, retained Beads tracker support, harmless Markdown emphasis/table formatting, and a one-line cleanup of `wizard`'s unused upstream `RED` shell variable.

All 25 mapped docs pages differ because this fork uses its own required Quickstart/source-page structure, names, and concise descriptions. Their behavior has been re-synced; upstream's longer FAQ and issue-history material was not copied wholesale. The 7 ignored upstream skills are still covered by active, matched ignore rules.
